...
下面面板的 **Accessibility** 分頁會自動檢查對比度、標籤這類問題：

- **Violations**：確定有問題，要修。目前所有元件都是 0，出現非零就值得回報
- **Passes**：通過的檢查，不用管
- **Inconclusive**：機器判不出來，需要人看一眼
...
- **`FlowChart` 底下的元件目前有已知問題**：它們的尺寸設定寫錯了，所以在 Storybook 裡會看起來扁掉或消失。這個已經記錄在案、等排修，看到不用回報。
