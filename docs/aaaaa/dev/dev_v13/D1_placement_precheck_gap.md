...
- [ ] §2 缺口敘述可獨立閱讀（0921 派工者不必回頭讀 layoutStore 原始碼）
- [ ] §3 簽章草案含 `DeviceDraft`／`LayoutView`／`DRAFT_ID` 與三個設計決定的理由
- [ ] §4 落點寫明「提共用、非另寫一份」，並列出 store 行為不變的驗收條件
- [ ] §5 切片建議標明 L1／L2 可平行開工
- [ ] §6 四個相鄰缺口各有現況、用途、不決理由
- [ ] 本版**未**動 `src/`
