...
FILENAME_RE = re.compile(r"^(\d{4})_(\d{8})_([a-z0-9][a-z0-9-]*)\.md$")
...
HIST_RE = re.compile(r"^\s*-\s+H(\d+)\s+·\s+(\d{4}-\d{2}-\d{2})\s+(\S+)\s+——\s*(\S.*)$")
HIST_MARKER = "**沿革**"
...
def parse_plan(path: Path, conflicts: list[Conflict]) -> Plan | None:
    ...
    if not m:
        conflicts.append(
            Conflict("BAD_FORMAT", path.name, "filename is not <seq>_<YYYYMMDD>_<topic>.md")
        )
        return None
