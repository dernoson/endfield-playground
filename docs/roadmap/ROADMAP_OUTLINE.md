...
| 09/27 | **M2 門檻** | **硬綁 B1**。B2／B4 維持不列必要（前置雖已就緒但本月只剩一週、兩項未開工）。**9/23 已裁：放行點擊落子**，選取／旋轉／刪除不開；#48 維持現狀；S1 擴為遷移＋樣式 |
...
| 版 | 日期 | 說明 |
|----|------|------|
| **v1.12** | **2026-09-23** | **主編裁決回寫（[PR #52](https://github.com/dernoson/endfield-playground/pull/52)）：** ①**R-B2 封鎖解除**——放行點擊落子，owner ＝ toby；選取／旋轉／刪除不開，B3／B4／B5 續鎖。②**R-D1 遷移封鎖撤銷**——未開 PR 的分支內容不算交付物，master 上只有一套 StatsPanel，不存在版本選擇問題；原判讀為誤判，§9.1 用詞一併更正。③**StatsPanel 單一 owner ＝ shirone**（元件＋路徑＋樣式），原屬 MBD 的樣式工作轉出，其 `dev/MBD` 分支不合入。④**#48 維持現狀不合不關**，落子鏈改在 master 版 `ToolbarPanel.vue` 上接；#48 日後合入的硬條件記入 [detail/B1](./detail/B1_toolbar_real_machines.md)。⑤「待審停滯 3 天點名」**不採納**，R-E2 不新增該條 |
