...
**第二項依賴解除：佈局殼。** 8/30 記的「仍等 B2／佈局渲染層落地」中，佈局殼一項已於本週達成——`GridCanvas`（#46）、`layoutStore`（#45）、主畫面接入（#50）全部在 master，佈局視角已是新殼。8/30 提的「先做已放置旋轉的獨立切片」現在不必再避開廢除清單，**可直接落在 `LayoutView.vue` 與 `layoutStore.rotateDevice`**。

**本項現在只剩 [B2](./B2_placement_chain.md) 一項前置**，而 B2 本身已由「等前置」改為「等裁決」。§8 的封鎖鏈因此縮為一層：**主編裁定落子鏈放行 → B2 實作 → 本項可開工。**

本項為 M2 加分（非必要），維持不派；B2 若依其 §11 改列 M3 前置，本項連帶順延至 10 月。
