...
「不要直接抱著整包機器資料」＝畫面不要直接綁整台 `Machine` 物件的欄位。  
**不是**叫你寫 unit test、也不是去做 detector／驗證規則。

主編 8/30：佈局會拔 Vue Flow 重刻。工單 [T1](./W0831-T1_inspector_flatten.md) 可選 **A 輕量攤平** 或 **B 等佈局**——請先看工單再動手。

---

## 1. 「攤平」是什麼（白話）

上週目標是：點設備 → 面板出現名稱、佔格、耗電（合入後點選可能仍沒反應——**不要本週修畫布**）。

現在的問題是：template 直接用 `selectedMachine.name`。`selectedMachine` 是整包機器資料。之後若要把「顯示」拆給別人（L3 只吃字），那一塊會開始挖 `machine.modes[0].xxx`，越挖越深。

**若選工單 A：** 先在同一個檔裡，算出一包**只有字串／數字的物件**，template **只綁這包**。查資料（store、`getMachine`）還是可以留在這個檔，因為它是容器。
